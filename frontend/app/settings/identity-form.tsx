...
export function IdentityForm({ initialIdentities }: { initialIdentities: Identity[] }) {
  ...
  return <form className="position-form identity-form" onSubmit={(event) => void save(event)}>
    <fieldset disabled={pending}>
      <legend>Your usernames and PGN player names</legend>
      {rows.map((row, index) => <div className="identity-row" key={row.key}>
        <label>Platform {index + 1}<select value={row.platform} onChange={(event) => update(row.key, 'platform', event.target.value)}><option value="lichess">Lichess</option><option value="chess_com">Chess.com</option><option value="other">Other</option></select></label>
        <label>{row.platform === 'other' ? 'PGN player name' : 'Username'} {index + 1}<input required maxLength={row.platform === 'other' ? 80 : 40} pattern={row.platform === 'other' ? undefined : '[A-Za-z0-9_\\-]+'} autoComplete="off" spellCheck={false} value={row.username} onChange={(event) => update(row.key, 'username', event.target.value)} /></label>
    ...
    <small>For Other, enter your player name from the PGN’s White or Black field, including spaces and punctuation. It matches games not recognized as Lichess or Chess.com.</small>
